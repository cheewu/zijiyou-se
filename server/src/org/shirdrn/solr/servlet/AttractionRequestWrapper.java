 ...
 * @date   2011-11-11
 */
public class AttractionRequestWrapper extends HttpServletRequestWrapper {

	public AttractionRequestWrapper(HttpServletRequest request){
		super(request);
	}
	...
	public Map<String,String[]> getParameterMap(){
		...
		for(String key : keys){
			...
			// 基于景点进行过滤
			if(key.equals(CommonParams.Q)) {
				fqValues = extendArrayValue(fqValues, "keyword:" + values[0]);
			} else if(key.equals(RequestParams.SEASON)){
				if(values.length<1) {
					continue;
				}
